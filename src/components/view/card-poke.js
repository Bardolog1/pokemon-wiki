...
export class CardPoke extends LitElement {
  ...
  toggleFlip() {
    ...
    if (this.pokemon && this.pokemon.name) {
      const canvasElement = this.shadowRoot.getElementById(
        `${this.pokemon.id}-chart`
      );
      if (canvasElement) {
  ...
  getTypeColors(type) {
    if (
      ...
      TYPES_COLORS[type] &&
      TYPES_COLORS[type].length === 2
    ) {
      return TYPES_COLORS[type];
    } else {
      return TYPES_COLORS["unknown"];
    }
  }
  ...
  getTypeNameForSVG(type) {
    if (
      ...
      this.pokemon.type.length > 0 &&
      Object.keys(TYPES_COLORS).includes(type)
    ) {
      return type;
    } else {
      return "pokemmo";
    ...
    if (Array.isArray(this.pokemon?.type)) {
      return this.pokemon.type.map(
        (item) => html`
          <span
            class="type"
            style="background: radial-gradient(circle, ${this.getTypeColors(
              item
            )[1]} 0%, ${this.getTypeColors(item)[0]} 100%);"
          >
  ...
  render() {
    ...
    this.firstType = this.pokemon?.type?.[0]
      ? this.pokemon?.type?.[0]
      : "unknown";
      ...
      <style>
        :host {
          --gradient-background: linear-gradient(
            90deg,
            ${this.getTypeColors(this.firstType)[0]} 0%,
            ${this.getTypeColors(this.firstType)[1]} 100%
          );
        }
      </style>
      ...
      <div class="scaff ${flipped ? "flipped" : ""}" id=${pokemon.id || 0}>
          ...
          <div class="card">
            ...
            <div class="image-container">
              <div class="back-container">
                <img
                  class="img-back"
                  src="assets/${this.getTypeNameForSVG(pokemon?.type?.[0])}.svg"
                  alt=""
                />
              </div>
              <img
                src="${pokemon.img ? pokemon.img : "assets/R.png"}"
                alt="${pokemon.name}"
              />
            </div>
            <div class="info-container">
              ...
              <div class="type-container">${this.renderTypeContainer()}</div>
        ...
        <div class="containerCardBack">
          <div class="card">
            ...
            <canvas id="${pokemon.id}-chart"></canvas>
          </div>
        </div>
      </div>
    `;
